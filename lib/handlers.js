...
handlers.vendor = (data, callback) => {
  if (data.method == "get") {

    // Get the filename neing requested
    const trimmedAssetName = data.path.replace("vendor/", "").trim();
...
handlers.index = (data, callback) => {
  if (data.method == "get") {
    ...
    const templateData = {
      "head.title": config.templateGlobals.appName,
      "head.description": "This is the meta description for Pizza Delivery App",
      "body.class": "index",
    };
...
handlers.signup = (data, callback) => {
  if (data.method == "get") {
    ...
    const templateData = {
      ...
      'head.description': 'Signup process is very easy here.',
      'body.class': 'signup'
    };
...
// Menu
handlers.itemsAll = (data, callback) => {
  if (data.method == "get") {
    // Prepare data for interpolation

    // let menuItems = '';

    // for (const key in config.menuItems) {
    //   if (config.menuItems.hasOwnProperty(key)) {
    //     const element = config.menuItems[key];
    //     const itemData = {
    //       'item.name': element.name,
    //       'item.price': element.price
    //     }

    //     helpers.getTemplate("_menu_items", itemData, (err, str) => {
    //       if (!err && str) {
    //         menuItems += str;
    ...
    //   }
    // }
    // callback(200, menuItems, "html");

    const templateData = {
      'head.title': 'Menu',
      'head.description': 'We have a wide variety Pizza\'s which are serving hot for you.',
      'body.class': 'menu',
      'menu.items': { 'template': '_menu_items', 'items': config.menuItems }
    };
...
handlers._users.post = (data, callback) => {
  const name =
    typeof data.payload.name === "string" && data.payload.name.trim().length > 0 ? data.payload.name.trim() : false;
  const email =
    typeof data.payload.email === "string" && helpers.validateEmail(data.payload.email.trim())
  ...
  const address =
    typeof data.payload.address === "string" &&
      ...
      data.payload.address.trim().length <= 200
  ...
  if (name && email && password && address && tosAgreement) {
    _data.read("users", email, (err, data) => {
      if (err) {
        ...
        callback(400, {
          Error: `A file with that email ${data.email} already exists`,
        });
      }
    });
  } else {
...
handlers._users.get = (data, callback) => {
  const email =
    typeof data.queryStringObject.email === "string" && helpers.validateEmail(data.queryStringObject.email.trim())
      ...
      if (tokenIsValid) {
        _data.read("users", email, (err, data) => {
          if (!err && data) {
            ...
            callback(500, {
              Error: `Some error while reading file ${email}.json`,
            });
          }
        });
      } else {
          ...
          if (!err && data) {
            _data.delete("users", email, err => {
              if (!err) {
                ...
                callback(500, {
                  Error: `Some deleting file ${email}.json`,
                });
              }
            });
          } else {
  ...
  if (id) {
    _data.read("tokens", id, (err, tokenData) => {
      if (!err && tokenData) {
        ...
        callback(500, {
          Error: `Some error while reading file ${id}.json`,
        });
      }
    });
  } else {
...
handlers._cart.post = (data, callback) => {
  const itemId =
    typeof data.payload.itemId === "number" && config.menuItems[data.payload.itemId] ? data.payload.itemId : false;
...
handlers._cart.put = (data, callback) => {
  ...
  const action =
    typeof data.payload.action === "string" && data.payload.action.trim().length > 0
      ? data.payload.action.trim()
      : false;
  const itemId =
    typeof data.payload.itemId === "number" && config.menuItems[data.payload.itemId] ? data.payload.itemId : false;
      ...
      if (!err && cartData) {
        ...
        handlers._tokens.verifyToken(token, cartData.userEmail, tokenIsValid => {
          if (tokenIsValid) {
            ...
            if (action == "update") {
              if (itemId && itemPosition == -1) {
                // If itemId not exists before in cart
              ...
              // in case of delete
              if (itemId && itemPosition > -1) {
                // If itemId already exists in cart
            ...
            _data.update("cart", id, cartData, err => {
              if (!err) {
                ...
                callback(500, {
                  Error: `Some error in updating file ${id}.json`,
                });
              }
            });
          } else {
          ...
          if (tokenIsValid) {
            _data.delete("cart", id, err => {
              if (!err) {
                ...
                callback(500, {
                  Error: `Some deleting file ${id}.json`,
                });
              }
            });
          } else {
...
handlers._order.post = (data, callback) => {
  const cartId =
    typeof data.payload.cartId === "string" && data.payload.cartId.trim().length > 0 ? data.payload.cartId : false;

  if (cartId) {
    // Validate Cart Id
    _data.read("cart", cartId, (err, cartData) => {
      if (!err && cartData) {
        const { userEmail } = cartData;
