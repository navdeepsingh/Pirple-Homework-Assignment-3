...
handlers._cart.post = (data, callback) => {
	const itemId =
    typeof data.payload.itemId === 'number' &&
    config.menuItems[data.payload.itemId]
    	? data.payload.itemId
    	: false;
...
handlers._cart.put = (data, callback) => {
    ...
    data.payload.action.trim().length > 0
